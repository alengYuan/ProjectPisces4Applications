...
import { useActionArea, useItemLibraryPathMP3 } from './model'
import { t } from '../../index'
 ...
 * @type {React.FC<{}>}
 */
const ItemLibraryPathMP3 = () => {
    const { isExpanded, switchPanel, status } = useItemLibraryPathMP3()
    ...
    return (
        <SettingsItemCollapsiblePanel
            ...
            icon=""
            title={t({
                en: 'Location of MP3 music library on device',
                zh: '设备上的 MP3 音乐库位置',
                ja: 'デバイス上の MP3 音楽ライブラリの場所',
            })}
            description={t({
                en: 'Specify the directory on the system for scanning and importing MP3 files, nested directories will not be scanned',
                zh: '指定系统上用于扫描和导入 MP3 文件的目录，不会扫描嵌套的目录',
                ja: 'MP3 ファイルのスキャンおよびインポートに使用するシステム上のディレクトリを指定します、ネストされたディレクトリはスキャンされません',
            })}
            status={status}
        >
...
export default memo(ItemLibraryPathMP3)